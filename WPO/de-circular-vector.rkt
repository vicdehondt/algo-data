...
(define-library (queue)
  ...
  (begin
    ...
    (define (enqueue-at-head! q val)
      ...
      (if (empty? q)
          (error "empty queue (enqueue-at-head!)" q))
      (let ((new-head (if (< (remainder (- (head q) 1) default-size) 1)
                          (- (vector-length (storage q)) 1)
                          (remainder (- (head q) 1) default-size))))
    ...
    (define (serve-at-rear! q)
      (if (empty? q)
          (error "empty queue (peek)" q))
      (let ((result (vector-ref (storage q) (rear q))))
        (rear! q (remainder (- (rear q) 1) default-size))
        result))))
